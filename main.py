...
USERS_FILE = "/tmp/users.json"  # Heroku-compatible
FILE_LOCK = asyncio.Lock()  # Prevent JSON write conflicts
...
async def init_referral(user_id: int, request: Request):
    ...
    if start_param and start_param.startswith("ref"):
        try:
            ...
            user, is_new = await get_or_create_user(user_id)
            if is_new and user["invited_by"] is None:
                async with FILE_LOCK:
                    async with aiofiles.open(USERS_FILE, mode='r') as f:
                        users = json.loads(await f.read())
...
async def watch_ad(user_id: int):
    ...
    if user.get("invited_by"):
        bonus_points = ad_points * 0.05  # 5% bonus
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
            ...
            if invited_by != user_id:  # Prevent self-referral
                ...
                if success:
                    ...
                    try:
                        ...
                        print(f"Failed to notify referrer {invited_by}: {e}")
                start_param = args[0]  # Pass to Mini App
        except ValueError:
    ...
    web_app_url = f"{BASE_URL}/app" + (f"?startapp={start_param}" if start_param else "")
